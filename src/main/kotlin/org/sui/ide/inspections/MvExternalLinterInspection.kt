...
import org.sui.ide.annotator.MvExternalLinterResult
import org.sui.ide.annotator.MvExternalLinterUtils
...
//        if (file !is MoveFile || file.containingCrate.asNotFake?.origin != PackageOrigin.WORKSPACE) return
    ...
    override fun inspectionFinished(
        ...
        while (true) {
            ...
            val futures = moveProjects.map {
                ApplicationManager.getApplication().executeOnPooledThread<MvExternalLinterResult?> {
    ...
    companion object {
        ...
        ): Lazy<MvExternalLinterResult?>? = runReadAction {
            val project = moveProject.project
            val suiCli = project.getSuiCli(disposable) ?: return@runReadAction null
            MvExternalLinterUtils.checkLazily(
                suiCli,
                project,
