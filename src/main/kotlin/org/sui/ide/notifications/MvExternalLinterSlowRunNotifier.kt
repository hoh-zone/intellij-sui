...
class MvExternalLinterSlowRunNotifier(val project: Project) {
    ...
    fun reportExecutionTime(executionTime: Long) {
        ...
        if (prevExecutionTimes.size == MAX_QUEUE_SIZE && minPrevExecutionTime > maxExecutionTime) {
            ...
            val widget = statusBar.getWidget(MvExternalLinterWidget.ID) as? MvExternalLinterWidget ?: return
    ...
    companion object {
        ...
        private const val DO_NOT_SHOW_KEY: String = "org.sui.external.linter.slow.run.do.not.show"
        private val LINTER_MAX_EXECUTION_TIME: RegistryValue = Registry.get("org.sui.external.linter.max.duration")
    }
}
