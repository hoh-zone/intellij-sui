...
import org.sui.openapiext.MvProcessResult
...
class FetchSuiPackageDialog(val project: Project) : DialogWrapper(project, true) {
    ...
    override fun doOKAction() {
        ...
        val downloadTask = object : Task.WithResult<MvProcessResult<ProcessOutput>, Exception>(
            project,
            "Downloading $accountAddress::$packageName...",
            true
        ) {
            override fun compute(indicator: ProgressIndicator): MvProcessResult<ProcessOutput> {
        ...
        if (decompile) {
            val decompileTask = object : Task.WithResult<MvProcessResult<ProcessOutput>, Exception>(
                project,
