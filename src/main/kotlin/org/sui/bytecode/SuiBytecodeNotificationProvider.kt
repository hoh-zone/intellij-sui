...
class SuiBytecodeNotificationProvider(project: Project) : EditorNotificationProvider {
    ...
    class DecompilationModalTask private constructor(
        ...
        Task.WithResult<MvResult<VirtualFile, String>, Exception>(
            project,
            "Decompiling ${file.name}...",
            true
        ) {

        override fun compute(indicator: ProgressIndicator): MvResult<VirtualFile, String> {
            return SuiBytecodeDecompiler().decompileFileToTheSameDir(sui, file)
        }

        fun runWithProgress(): MvResult<VirtualFile, String> = ProgressManager.getInstance().run(this)
