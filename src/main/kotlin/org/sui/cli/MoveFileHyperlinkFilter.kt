...
class MoveFileHyperlinkFilter(
    ...
    override fun applyFilter(line: String, entireLength: Int): Filter.Result? {
        ...
        val filePath = match.groups[1] ?: return null
        val lineNumber = match.groups[2] ?: return null
        val columnNumber = (if (match.groups.size > 3) match.groups[3] else null)
            ?: MatchGroup("1", IntRange(lineNumber.range.last, lineNumber.range.last))

        val lineStart = entireLength - line.length
        val startInd = lineStart + filePath.range.first
