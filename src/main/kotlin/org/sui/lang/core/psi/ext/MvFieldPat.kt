...
val MvPatField.fieldReferenceName: String
    get() = patFieldFull?.referenceName ?: patBinding?.referenceName.orEmpty()
...
val MvPatField.kind: PatFieldKind?
    get() = patBinding?.let { PatFieldKind.Shorthand(it) }
        ?: patFieldFull?.let { PatFieldKind.Full(it.referenceNameElement, it.pat) }
...
val PatFieldKind.fieldName: String
    get() = when (this) {
        ...
        is PatFieldKind.Shorthand -> binding.name.orEmpty()
    }
