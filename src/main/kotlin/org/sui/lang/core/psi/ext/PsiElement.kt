...
// 定义一个内联函数，用于获取当前 PsiElement 的严格祖先元素
// 该函数是泛型函数，T 是 PsiElement 的子类
// reified 关键字表示 T 类型在运行时是可用的，可以在函数内部访问
inline fun <reified T : PsiElement> PsiElement.ancestorStrict(stopAt: Class<out PsiElement>): T? =
    // 调用 PsiTreeUtil 的 getParentOfType 方法来获取指定类型的祖先元素
    // this 表示当前 PsiElement
    // T::class.java 表示要查找的祖先元素的类型
    // true 表示严格查找，即不包括当前元素本身
    // stopAt 表示查找的终止条件，遇到该类型的祖先元素即停止查找
