...
//        rawCachedMultiResolve()
    ...
    private fun rawMultiResolveUsingInferenceCache(): List<MvPathResolveResult<MvElement>>? {
        ...
        return pathElement.inference(msl)?.getResolvedPath(pathElement.path)
            ?.map {
                MvPathResolveResult(it.element, it.isVisible)
            }
    }
...
//fun resolveAliases(processor: MvResolveProcessor): MvResolveProcessor =
