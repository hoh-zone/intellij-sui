            ...
            is Restricted -> {
                when (visibility) {
                    ...
                    is Restricted.Package -> {
                        ...
                        val pathPackage = methodOrPath.containingMovePackage
                        val itemPackage = item.containingMovePackage
                        if (pathPackage != null && itemPackage != null) {
                            // Normal package-aware visibility check.
                            if (pathPackage == itemPackage) Visible else Invisible
                        } else {
                            // Fallback for detached/library files where Move package model isn't available.
                            // In that case, avoid false negatives for same-address std modules
                            // (e.g. std::u8 calling std::macros public(package) macros).
                            val pathAddress = pathModule?.addressRef?.text
                            val itemAddress = itemModule?.addressRef?.text
                            if (pathAddress != null && pathAddress == itemAddress) Visible else Invisible
                        }
                    }
                }
            }
