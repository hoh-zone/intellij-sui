...
import org.sui.utils.recursionGuard
...
typealias RelateResult = MvResult<Unit, CombineTypeError>

private inline fun RelateResult.and(rhs: () -> RelateResult): RelateResult = if (isOk) rhs() else this

typealias CoerceResult = MvResult<CoerceOk, CombineTypeError>
...
private val EMPTY_INFERENCE_RESULT: InferenceResult = InferenceResult(
    patTypes = emptyMap(),
    patFieldTypes = emptyMap(),
    exprTypes = emptyMap(),
    exprExpectedTypes = emptyMap(),
    methodOrPathTypes = emptyMap(),
    resolvedPaths = emptyMap(),
    resolvedFields = emptyMap(),
    resolvedMethodCalls = emptyMap(),
    resolvedBindings = emptyMap(),
    resolvedLitFields = emptyMap(),
    callableTypes = emptyMap(),
    typeErrors = emptyList()
)

fun inferTypesIn(element: MvInferenceContextOwner, msl: Boolean): InferenceResult {
    return EMPTY_INFERENCE_RESULT
}
...
class InferenceContext(
    ...
    fun infer(owner: MvInferenceContextOwner): InferenceResult = EMPTY_INFERENCE_RESULT
...
//        this.freezeUnification { this.combineTypes(ty1, ty2) }
    ...
    fun combineTypesNoVars(ty1: Ty, ty2: Ty): RelateResult {
        return when {
            ...
            // Move style tolerance: `(expr,)` is often used as multiline formatting.
            // Treat single-element tuple as equivalent to the inner type in type matching.
            ty1 is TyTuple && ty1.types.size == 1 && ty2 !is TyTuple ->
                combineTypes(ty1.types[0], ty2)
            ty1 !is TyTuple && ty2 is TyTuple && ty2.types.size == 1 ->
                combineTypes(ty1, ty2.types[0])
